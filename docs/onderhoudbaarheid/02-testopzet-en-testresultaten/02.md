...
| Regressietests | — | de volledige bestaande testsuite, vóór/na de verbeteringen | gelijkblijvend gedrag (nulmeting voor [`06`](../06-validatie-verbeteringen/06.md)) |
...
**Functie.** Dit is de *default* zoekhandler voor het endpoint `GET /ws/rest/v1/concept` en vindt *concepten* op drie elkaar uitsluitende manieren: (1) op bron + code (`source`/`code`), (2) op naam + klasse (`name`/`class`/`searchType`, met `equals` of `fuzzy`), of (3) via een lijst referenties (`references` — UUID's of `bron:code`-mappings). Dat al deze zoekmodi in één `search()` zitten, verklaart de hoge complexiteit.
...
Optellen van alle beslispunten incl. `&&`/`||` geeft **CC ≈ 28** voor `search()`: D1, de referentie-lus (D2–D6, met D5 een `&&`), D7, D8, D9 + fuzzy-lus, D10 (`||`), D11, D12, de namen-lus (D13 met `&&` + `||`), D14, D15–D17 en de mapping-lus (D18, `||`). JaCoCo rapporteert **30** voor de hele klasse, de cognitive complexity is **31** — *zeer hoog risico* (> 25).
...
Naast de white-box units draaien we de bestaande, uitgebreide testsuite van de module (controllertests per resource onder `omod/src/test/...`, en de IT's in [`integration-tests`](../../../integration-tests/)). Deze tests benaderen de module als geheel via de REST-API (black-box) en vormen samen de **regressie-nulmeting**: ze worden vóór de refactoring vastgelegd en ná de refactoring opnieuw gedraaid om aan te tonen dat extern gedrag ongewijzigd blijft (zie [`06-validatie-verbeteringen`](../06-validatie-verbeteringen/06.md)).
...
De onderstaande cijfers komen uit het JaCoCo-rapport dat met `mvn verify` is gegenereerd (run van **2026-06-12**, `omod`-submodule, `omod/target/site/jacoco/jacoco.csv` — hetzelfde commando dat de CI-workflow draait). Ze vormen de **nulmeting** vóór de verbeteringen en zijn de basis voor de voor/na-vergelijking in [`06-validatie-verbeteringen`](../06-validatie-verbeteringen/06.md).
...
> Reproduceren: `mvn -B -ntp verify -Dmaven.test.failure.ignore=true` (de `-Dmaven.test.failure.ignore` is nodig omdat de oudere OpenMRS-versie enkele tests structureel laat falen — zie de CI-workflow). Het rapport verschijnt in `omod/target/site/jacoco/index.html`.
