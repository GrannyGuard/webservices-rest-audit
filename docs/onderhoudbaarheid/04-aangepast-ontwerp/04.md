...
Cognitive complexity: **31** per functie (zeer hoog risico, > 25 volgens SIG/TÜViT).
...
```mermaid
classDiagram
    class ConceptSearchHandler1_8_voor {
        +getSearchConfig() SearchConfig
        +search(context) PageableResult
    }
    note for ConceptSearchHandler1_8_voor "search(): cognitive complexity 31\nAlle zoekmodi inline in één methode:\n- references (UUID + source:code lus)\n- fuzzy name\n- exact name (naamlus + null-check)\n- source/code mapping\nGeen private helpermethoden"
```

```mermaid
classDiagram
    class ConceptSearchHandler1_8 {
        +getSearchConfig() SearchConfig
        +search(context) PageableResult
        -searchByReferences(context, conceptReferences) PageableResult
        -searchByFuzzyName(context) PageableResult
        -searchByExactName(context, name) PageableResult
        -searchBySourceMapping(context) PageableResult
    }
    note for ConceptSearchHandler1_8 "search(): dispatcher CC ≈ 4\nElke zoekmodus in eigen private methode"
```
...
### Toegepaste ontwerpprincipes (deze handler)

| Principe | Toepassing |
|---|---|
| **Extract Method** (Fowler) | Vier zoekmodi elk naar een eigen private methode; de naam vervangt commentaar door zelfverklarende code |
| **Single Responsibility Principle** | Elke methode heeft één reden om te veranderen (bv. wijziging in fuzzy-zoeklogica raakt alleen `searchByFuzzyName`) |
| **Guard Clauses** | `search()` gebruikt vroege returns i.p.v. de originele diep geneste `if`/`else if`/`else`-keten; ongeldig `searchType` wordt nu eerder en explicieter afgevangen |
| **Bugfix als bijvangst** | Origineel: `concepts.add(concept)` staat vóór de null-check, waardoor bij een onbekend concept `null` in de lijst belandt. Na refactoring retourneert `searchByExactName` direct `EmptySearchResult()` als `concept == null` |

Strategy en Open/Closed Principle zijn hier bewust **niet** toegepast; zie §4 voor de motivatie.
...
- Extract Method brengt alle methodes al onder de SIG-grens van 15 (zie tabel §1). De Strategy-variant zou de cognitive complexity niet verder verbeteren, maar wel extra bestanden, Spring-bean-configuratie en indirectie toevoegen: kosten zonder aantoonbare meerwaarde voor de gestelde kwaliteitseis.
...
**Niet gekozen, omdat:**
- De drie takken verschillen **uitsluitend** in het matchpredicaat (regel 108/115/122 in de huidige code); de resolutie- en paginalogica eromheen is identiek. Een volledige strategy per datatype zou die gedeelde logica dupliceren en daarmee een nieuwe duplicatiebron introduceren (zie de bevinding in [03](../03-verbeteringen-prioritering/03.md#niet-opgepakt-met-onderbouwing) over duplicatie).
- Strategy op het niveau van het matchpredicaat (`ObsValueMatcher`) isoleert precies het deel dat varieert (Open/Closed) zonder de rest van de methode te splitsen. Dat is een kleinere, gerichtere wijziging die beter aansluit bij de "beperkte effort"-inschatting uit 03.
