...
Om de onderhoudbaarheid van de gekozen OpenMRS REST Webservices module te beoordelen, hebben we SonarCloud gebruikt. SonarCloud analyseert de broncode automatisch en levert metrieken op het gebied van complexiteit, duplicatie, code smells en technische schuld. De resultaten van deze analyse vormen de basis voor de beoordeling van de onderhoudbaarheid van de module en zijn gekoppeld aan de ISO 25010 kwaliteitskenmerken analyseerbaarheid, aanpasbaarheid, testbaarheid en modulariteit.
...
Hoge cyclomatic complexity raakt direct twee ISO 25010-kenmerken: *testbaarheid* (elke extra branch vereist een extra testgeval; klassen met CC > 50 hebben tientallen gevallen nodig voor volledige branch coverage) en *aanpasbaarheid* (meer paden = grotere kans op regressie bij wijzigingen).
...
De totale cognitive complexity van de module bedraagt **3.621**, met een gemiddelde van **1,44 per functie**. Dat is opnieuw laag, maar de verdeling is scheef: de `omod`-map neemt **2.324** (64%) voor zijn rekening en `omod-common` **1.297** (36%) van het totaal.
...
Per functie bekeken verschuift het beeld flink. `ConceptSearchHandler1_8.java` heeft maar 2 functies, met een gemiddelde cognitive complexity van **31,0**, ruim boven de SIG-grens van 25 (zeer hoog risico). `EncounterSearchHandler1_9.java` zit op **17,6** per functie (hoog risico). Deze SearchHandler-klassen zijn dus klein van omvang, maar bevatten functies die afzonderlijk extreem moeilijk te begrijpen en te onderhouden zijn.
...
Hoge cognitive complexity raakt vooral de *analyseerbaarheid* (diep geneste code is veel lastiger te volgen bij het zoeken naar bugs) en de *aanpasbaarheid*, omdat complexe logica de kans op onbedoelde neveneffecten bij wijzigingen vergroot.
...
Voor ISO 25010 zit het probleem bij *aanpasbaarheid*: een bugfix moet op elke kopie worden doorgevoerd, en als je er één mist heb je meteen een inconsistentie. Daarnaast maakt de extra hoeveelheid code het moeilijker te bepalen welke kopie de "echte" logica bevat (*analyseerbaarheid*).

---

## Code smells en technische schuld

Een *code smell* is geen bug (de code werkt gewoon), maar een patroon in de broncode dat wijst op een zwakke structuur en het onderhoud lastiger en risicovoller maakt. Bekende voorbeelden zijn te lange methodes, te grote klassen, gedupliceerde code, te diep geneste logica en dode (ongebruikte) code. Een smell breekt op zichzelf niets, maar vergroot de kans op fouten zodra de code later wordt gewijzigd.
...
Smells als lange methodes en dode code maken het lastiger om code snel te begrijpen (*analyseerbaarheid*), en de opgebouwde schuld maakt elke toekomstige wijziging risicovoller en duurder (*aanpasbaarheid*).
