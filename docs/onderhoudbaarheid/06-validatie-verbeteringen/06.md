...
risico). Geen enkele methode valt in de categorie *hoog risico* (> 10) — de schattingen uit 04 komen daar goed mee overeen.
...
zit de branch coverage van alle drie de matchers + de factory nu op **100 %** (zie §4).
...
`ObsValueMatcher`-implementaties + de factory zijn dankzij `ObsValueMatcherTest` (9 cases,
...
en draait daardoor in ~2 s i.p.v. de ~35 s die de context-sensitieve handler-tests kosten —
een bijkomend onderhoudbaarheidsvoordeel van de Strategy-extractie: de datatype-matchlogica is
nu los van de REST-/persistentielaag te testen.
...
- In `ConceptSearchHandler1_8` zat een latente null-pointer bug: de originele code deed `concepts.add(concept)` vóór de null-check, waardoor `null` in de lijst kon belanden. De geëxtraheerde `searchByExactName` returnt bij `concept == null` direct `EmptySearchResult()` — dat is nu opgelost.
...
- De Strategy-extractie maakte drie latente null-pointer-paden zichtbaar en testbaar: `obs.getValueCoded()`, `obs.getValueNumeric()` (auto-unboxing `Double`→`double`) en `obs.getEncounter()` konden alle drie `null` opleveren. Ze zijn gehard in commit `46cd46b` (SonarCloud `java:S2259`) en zijn nu — dankzij `ObsValueMatcherTest` — ook expliciet door tests afgedekt. Het loskoppelen van de matchlogica van de REST-/persistentielaag is hier de onderhoudbaarheidswinst: deze randgevallen waren vóór de refactoring alleen via een volledige context-sensitieve integratietest te bereiken.

De basis-path testsets uit 02 (TC1–TC11 en TC1–TC10) draaien na de refactoring zonder aanpassingen door — de publieke `search(RequestContext)`-signatuur is ongewijzigd.
