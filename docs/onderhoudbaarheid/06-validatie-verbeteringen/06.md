...
De schattingen uit [`04`](../04-aangepast-ontwerp/04.md) liggen iets lager dan de gemeten 39,
omdat de tool sommige nesting-constructies zwaarder weegt. De verdeling is hier informatief; de
...
risico). Geen enkele methode valt in de categorie *hoog risico* (> 10), en de schattingen uit 04 komen daar goed mee overeen.
...
|---|---|---|---|
| `search()` (dispatcher) | **6** | 93 % | Gemiddeld risico |
...
| `NumericObsValueMatcher` (matches) | ≤ 2 | **100 %** | Laag risico |
| `CodedObsValueMatcher` (matches) | 1 | **100 %** | Laag risico |
| `TextObsValueMatcher` (matches) | 1 | n.v.t. (geen branches) | Laag risico |
...
die elk triviaal (CC ≤ 2) zijn. De matcher-klassen zijn in PR #105 voorzien van **dedicated
unit tests** (`ObsValueMatcherTest`, 9 cases) die ook de null-/randgevallen dekken. Daardoor
zit de branch coverage van alle drie de matchers plus de factory nu op **100 %** (zie §4).
...
`omod`-suite: **1.854 tests, 0 failures, 0 errors, 14 skipped**) toont het resultaat tegenover
de nulmeting uit 02.
...
| Ná — handler | 28/30 — **93,3 %** | 56/56 — 100 % | **2** |
| Ná — handler + `ObsValueMatcher`-klassen | 36/38 — **94,7 %** | — | **2** |

Het aantal branches van de handler zelf daalde van 44 naar 30: de drie datatype-takken
...
Een nuance bij de handler-rij: de twee ongedekte branches zitten ná PR #105 **volledig in de
handler zelf** (de defensieve `currEncounter != null`-guard in `getEncountersForObs`, een
NPE-hardening uit commit `46cd46b` die in de testdata nooit `null` raakt). De drie
`ObsValueMatcher`-implementaties plus de factory zijn dankzij `ObsValueMatcherTest` (9 cases,
incl. de null-/non-numeric-randgevallen) nu **100 % branch-covered**. Vóór die testklasse
stonden `NumericObsValueMatcher` op 66 % en `CodedObsValueMatcher` op 50 %.
...
| `ObsValueMatcherTest` | `f8d3b70` | 9 (Strategy-units: match/no-match + null-/non-numeric-randgevallen) | ✅ 9/9 geslaagd |

Lokale run: `mvn -pl omod test -Dtest="ConceptSearchHandler1_8Test,EncounterSearchHandler1_9Test,ObsValueMatcherTest"` →
**31 tests, 0 failures, 0 errors, BUILD SUCCESS**. De volledige `omod`-suite telt na PR #105
**1.854 tests (0 failures, 0 errors, 14 skipped)**; zie §4.

De `ObsValueMatcherTest` is een **pure unit test** (plain `Obs`-objecten, geen Spring-context)
en draait daardoor in ~2 s in plaats van de ~35 s die de context-sensitieve handler-tests kosten.
Dat is een bijkomend onderhoudbaarheidsvoordeel van de Strategy-extractie: de datatype-matchlogica
is nu los van de REST-/persistentielaag te testen.
...
### Module-niveau — Maintainability Rating blijft A

Naast de per-klasse metingen hierboven dient de module-brede **SonarCloud Maintainability
Rating** als regressievangnet op het hoogste niveau. De nulmeting in
[`01`](../01-analyse-onderhoudbaarheid/01.md) rapporteerde een **Rating A** (debt ratio
4,4%). Ná de refactoring rapporteert SonarCloud nog steeds een **Rating A**: de twee
gerichte ingrepen hebben de technische schuld op klasseniveau verlaagd zonder de
module-brede rating te laten zakken. Er is dus ook op het hoogste aggregatieniveau geen
regressie opgetreden.

![SonarCloud maintainability — module-brede Rating A na de refactoring](images/sonarcloud-maintainability.png)

*Figuur 4: SonarCloud-nameting op module-niveau — de Maintainability Rating blijft A.*
...
De zwaarste methoden (cognitive CC 31 resp. 17,6) bestaan niet meer; geen enkele methode zit nog boven cyclomatische complexity 10.

Twee dingen vielen verder op tijdens de refactoring:

- In `ConceptSearchHandler1_8` zat een latente null-pointer bug: de originele code deed `concepts.add(concept)` vóór de null-check, waardoor `null` in de lijst kon belanden. De geëxtraheerde `searchByExactName` returnt bij `concept == null` nu direct `EmptySearchResult()`, dus dat is opgelost.
- Bij `EncounterSearchHandler1_9` is het Open/Closed Principle een bijvangst van de Strategy: een nieuw obs-datatype voeg je toe met alleen een nieuwe `ObsValueMatcher`, zonder `search()` aan te raken.
- De Strategy-extractie maakte drie latente null-pointer-paden zichtbaar en testbaar: `obs.getValueCoded()`, `obs.getValueNumeric()` (auto-unboxing `Double`→`double`) en `obs.getEncounter()` konden alle drie `null` opleveren. Ze zijn gehard in commit `46cd46b` (SonarCloud `java:S2259`) en worden nu, dankzij `ObsValueMatcherTest`, ook expliciet door tests afgedekt. De onderhoudbaarheidswinst zit hier in het loskoppelen van de matchlogica van de REST-/persistentielaag: deze randgevallen waren vóór de refactoring alleen via een volledige context-sensitieve integratietest te bereiken.

De basis-path testsets uit 02 (TC1–TC11 en TC1–TC10) draaien na de refactoring zonder aanpassingen door, omdat de publieke `search(RequestContext)`-signatuur ongewijzigd is.
