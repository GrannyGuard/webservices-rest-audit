...
> **Afdwinging (niet alleen geconfigureerd):** het `prod`-environment is gekoppeld
> aan de release-pipeline. De job `build-sign-attest` in
> [`release-sign.yml`](../../../.github/workflows/release-sign.yml) draait
> `environment: prod` en produceert het productie-gebonden (gesigneerde) artefact.
> Daardoor vuurt de approval-gate (required reviewers `DanielvG-IT` / `BaasW`)
> én de `protected_branches`-policy (alleen runs vanaf `main`) daadwerkelijk
> vóórdat er getekend of gepubliceerd wordt — de OTAP-scheiding is dus technisch
> afgedwongen, niet enkel gedocumenteerd. De scan-/quality-workflows (CodeQL,
> SonarCloud, SBOM) draaien bewust *zonder* environment: het zijn read-only
> analyses op elke push/PR en mogen niet achter een approval-gate komen.
