# Formeel auditrapport

Rubric: *"Verbeteronderzoek security"*, Sprint 4-eindoplevering.

---

## TODO's

- [ ] **Executive Summary** schrijven: max. 1 pagina voor management
- [ ] **Scope en Context** beschrijven: wat onderzocht, wat buiten scope, en het wettelijk kader (AVG, WGBO, NEN-7510:2024-2)
- [ ] **Audit Methodologie** beschrijven: aanpak, tooling, testmethode
- [x] **Risico-analyse** opnemen met ten minste 4 bevindingen, met per bevinding beschrijving, risicoscore, impact en aanbeveling (H4 geschreven: risicomatrix + 5×5-grid + top-11 risico's; 5 bevindingen B-001 t/m B-005, waarvan B-005 gemitigeerd+geverifieerd)
- [x] **SBOM en Supply Chain Security** samenvatten: verwijzen naar `03-sbom-en-cve-advies/` (H5 + Bijlage C geschreven; `sbom.cdx.json` gevuld met echte CycloneDX 1.6 SBOM, 237 componenten)
- [ ] **Conclusie en Advies** schrijven: geprioriteerde verbeteracties
- [ ] **Bijlagen toevoegen:** traceability matrix, SBOM (CycloneDX JSON), SAST-output, risicomatrix, bow-tie diagrammen, Snyk-rapport, CRA-mapping
