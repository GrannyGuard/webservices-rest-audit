...
\textbf{Status}      & \amber{Deels gemitigeerd} --- CWE-302 gefixt, CWE-384 (sessie-fixatie) nog open; prio.\ 2; bow-tie in \texttt{01.md} \S4.3 \\
\end{tabular}
...
zonder server-side validatie; \texttt{:119} --- na \texttt{Context.authenticate()}
wordt de sessie niet geïnvalideerd (CWE-384, nog open). De afzonderlijke CWE-302
fall-through --- de ontbrekende \texttt{return} na de \texttt{sendError(401)} bij een
verlopen sessie, waardoor de keten dóórliep naar \texttt{chain.doFilter()} --- is
inmiddels \groen{gefixt} (er staat nu een \texttt{return} op \texttt{:92}). De
CodeQL-melding \texttt{java/user-controlled-bypass} (alert \#908) op de optionele
Basic-Auth-tak is na onderzoek als geaccepteerd risico (false positive)
geclassificeerd --- zie \texttt{06-mitigatie-en-validatie/06.md} \S3b.
...
\textbf{Bewijs.} \texttt{AuthorizationFilter.java:119} ---
...
\textbf{Validatie (herscan).} De CodeQL-herscan op \texttt{main} (run
\texttt{27471405779}, commit \texttt{b5627faa} via PR~\#96, 2026-06-13) zet
\texttt{java/xss} alert~\#1 op \texttt{state: fixed} --- net als bij B-005 is de
detect-\,$\to$\,mitigeer-\,$\to$\,verifieer-cyclus daarmee kwantitatief en
reproduceerbaar afgesloten.
...
\begin{tabularx}{\textwidth}{@{}l X X@{}}
    ...
    \textbf{Deze sprint} & Gemiddelde bevinding, of een hoge bevinding die al
                  deels gemitigeerd is: B-002 (CWE-302 gefixt, CWE-384 open) en
                  de SBOM-quickwins. &
