...
\begin{itemize}
    ...
    \item \textbf{Fysieke beveiliging en organisatorische maatregelen} ---
          ...
          niet beoordeeld.
    \item \textbf{Penetratietests op productie} --- er is geen geautoriseerde
          productieomgeving beschikbaar; de uitgevoerde penetratietests zijn
          daarom beperkt tot de eigen test-/dev-omgeving (fork) en zijn
          afzonderlijk vastgelegd in
          \texttt{docs/security/05-penetration-tests/PentestReport-OpenMRS.md}.
\end{itemize}
...
Elke bevinding is handmatig geverifieerd tegen de broncode (bestand + regel). De
\texttt{CR-x}-verwijzingen koppelen elke bevinding aan de bijbehorende code-review-bevinding
in \texttt{04-code-review/04.md}; deze nummering staat los van de \texttt{F-xx}-bevindingen
uit het afzonderlijke penetratietestrapport.
...
\subsection{B-002 --- Sessie-fixatie en best-effort toegangsfilter (SQ1, CR-3/CR-7)}
\label{sec:b002}
...
\subsection{B-003 --- Ontbrekende brute-force-bescherming op authenticatie (CR-8)}
\label{sec:b003}
...
\subsection{B-004 --- Reflected XSS in het Swagger debug-endpoint (SQ8, CR-2)}
\label{sec:b004}
...
\subsection{B-005 --- Log injection (CWE-117): gemitigeerd en geverifieerd (CQL1, CR-1)}
\label{sec:b005}
...
Deze bijlage koppelt tien NEN-7510:2024-2 controls aan een traceerbaar
bewijsartefact (repository-instelling, CI-workflow, commit of scan-output),
ruim boven de Sprint-4-vereiste van minimaal drie controls. De onderhouden
bronversie van deze matrix staat in de repository:
\texttt{docs/security/traceability-matrix.md}; deze bijlage volgt die bron.
Sectieverwijzingen (\S) verwijzen naar de onderliggende documentatie in
\texttt{docs/security/}.
...
Twee-account-DB-model: een privileged migratie-account (Liquibase/DDL) gescheiden
van een DML-only runtime-account (\texttt{SELECT/INSERT/UPDATE/DELETE}, kan nooit
\texttt{DROP}). &
\texttt{docker/db-init/10-runtime-least-privilege.sh},
\texttt{docker-compose.yml}; hardening checklist \S2. &
PoC-mechanisme; prod-runtime-switch = deploy-stap \\
\addlinespace

\textbf{A.8.20 / A.8.26} Host-validatie (Swagger) &
OpenAPI-spec reflecteert \texttt{Host}/\texttt{Scheme} niet langer ongevalideerd:
Host-allow-list + scheme-sanitisatie (SQ9-fix); Swagger-endpoints uitschakelbaar
(SQ8-blootstelling). &
\texttt{RestUtil.resolveSwaggerHost()} / \texttt{sanitizeScheme()}; unit tests
\texttt{SwaggerHardeningRestUtilTest} (8/8 groen); hardening checklist \S1, \S10. &
Geïmplementeerd + getest (SQ9); SQ8-encoding open \\
\addlinespace

\textbf{A.5.23 / A.8.32} Supply chain / change management &
Verifieerbare releases: keyless cosign-signing + SLSA build-provenance +
CycloneDX-SBOM naast de \texttt{.omod}; alle CI-actions SHA-pinned. &
\texttt{.github/workflows/release-sign.yml}; hardening checklist \S8, \S9. &
...
\texttt{RestUtilTest}; fix commit \texttt{831dafb}. &
Geïmplementeerd + getest \\
\addlinespace

\textbf{A.8.31} Scheiding ontwikkel-/test-/prod-omgevingen &
Aparte compose-overrides per omgeving + GitHub Environments (dev/test/prod) met
gescheiden secrets; prod publiceert de DB niet en faalt fast op ontbrekende
secrets. &
\texttt{docker/\{dev,test,prod\}/docker-compose.yml}; \texttt{02.md} \S2. &
Geïmplementeerd \\
...
\begin{itemize}
    ...
    \item Control A.8.29 (beveiligingstests) in de traceability matrix
          opnemen met het afzonderlijke penetratietestrapport
          (\texttt{05-penetration-tests/}) als bewijsartefact.
\end{itemize}
