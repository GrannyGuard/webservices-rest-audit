...
\begin{itemize}
    ...
    \item \textbf{Penetratietests op een live productiesysteem met echte
          ...
          De penetratietest is wél uitgevoerd op een geïsoleerde VirtualBox-testomgeving
          (Kali Linux 2024, OpenMRS~2.8.7, Apache Tomcat~9.0.118) en heeft
          9~bevindingen opgeleverd (zie Bijlage~\ref{app:pentest}).
\end{itemize}
...
Hieronder zijn zes bevindingen uitgewerkt, gesorteerd op risicoscore (B-001 het
hoogst) en gespreid over de geauditeerde controls. Vijf staan nog open
(B-001 t/m B-005); B-006 toont een afgesloten
detect-\,$\to$\,mitigeer-\,$\to$\,verifieer-cyclus.
Alle bevindingen zijn empirisch geverifieerd via pentest of SAST-tooling.
...
\textbf{Status}      & \rood{Open --- directe actie (Nu)}; bow-tie in \texttt{01.md} \S4.3 \\
\end{tabular}
...
\subsection{B-004 --- Reflected XSS in het Swagger debug-endpoint (SQ8 / CodeQL \texttt{java/xss})}
\label{sec:b004}
...
\textbf{Status}      & \amber{Code-fix toegepast --- CodeQL-herscan als bewijsartefact nog toe te voegen} \\
\end{tabular}
...
\textbf{Mitigatie.} \texttt{tag} wordt nu HTML-geëncodeerd via Spring
\texttt{HtmlUtils.htmlEscape(tag)} vóór het in de respons wordt geplaatst
(\texttt{SwaggerDocController.java:27}); \texttt{mvn clean install} slaagt.
Defense-in-depth: de \texttt{/apiDocs}-boom is uitschakelbaar via
\texttt{webservices.rest.enableSwaggerDocs=false} (HTTP~404 in productie).
Restpunt: CodeQL-herscan toevoegen als bewijsartefact zodat alert~\#1
aantoonbaar op \texttt{state: fixed} staat (zelfde patroon als B-006).
...
De centrale auditvraag luidt: \emph{Voldoet de OpenMRS REST Webservices Module
(v3.2.0) aan de geselecteerde NEN-7510:2024-2 controls, en wat moet er veranderen
om dat te bereiken?}

Het antwoord is: \textbf{gedeeltelijk}. Twee van de zeven geauditeerde controls
zijn aantoonbaar geïmplementeerd; de overige vijf kennen significante gaps.
Onderstaande tabel vat de compliancestatus per control samen.

\vspace{0.4em}
{\small
\begin{tabularx}{\textwidth}{@{}lXl@{}}
\toprule
\textbf{Control} & \textbf{Compliancestatus} & \textbf{Bev.} \\
\midrule
A.8.3 Toegangsbeveiliging &
    \amber{Gedeeltelijk} --- geen declaratieve access control op REST-laag;
    sessie-fixatie (CWE-384) maakt sessie-overname mogelijk &
    B-002 \\
A.8.5 Authenticatie &
    \rood{Niet compliant} --- geen rate-limiting (pentest: credentials in
    6~pogingen), sessie-fixatie (CWE-384 open), gebruikersenumeratie via lockout
    side-channel &
    B-001, B-002, B-003 \\
A.8.8 Kwetsbaarheden &
    \amber{Gedeeltelijk} --- SBOM en scanning ingericht; 75~CVE's geïdentificeerd,
    quick-win patches nog niet doorgevoerd &
    \S\ref{ch:sbom} \\
A.8.9 Configuratiebeheer &
    \amber{Niet geverifieerd via pentest} --- gap gedocumenteerd in gap-analyse
    (01.md §3.5); empirisch bewijs ontbreekt &
    01.md §3.5 \\
A.8.15 Logging &
    \groen{Grotendeels compliant} --- zes logging-gaps gedicht (commit \texttt{8521cdc}),
    log injection gemitigeerd (commit \texttt{831dafb}, CodeQL 9\,$\to$\,0 alerts);
    resterend: stack trace disclosure nog open &
    B-005, B-006 \\
A.8.25 Veilige SDLC &
    \groen{Compliant} --- branch protection, CodeQL als verplichte status check,
    verplichte PR-reviews &
    Bijlage~\ref{app:traceability} \\
A.8.29 Beveiligingstests &
    \amber{Gedeeltelijk} --- pentest uitgevoerd (9 bevindingen); hervalidatie na
    mitigaties deels in uitvoering &
    Bijlage~\ref{app:pentest} \\
\bottomrule
\end{tabularx}
}

\vspace{0.6em}
De module verwerkt bijzondere medische persoonsgegevens (AVG Art.~9,
WGBO Art.~7:454). De open kwetsbaarheden in A.8.3 en A.8.5 zijn daarmee
\textbf{niet acceptabel}: B-001/B-002 (score~20/15) en B-003 (score~12,
raakt KJ3) kunnen bij misbruik leiden tot een meldplichtig datalek
(AVG Art.~33) of directe aantasting van de patiëntveiligheid. De prioriteit
ligt bij het beveiligen van het authenticatiemechanisme (A.8.5): rate
limiting, sessie-ID-rotatie na login en het tegengaan van
...
Deze bijlage koppelt zeven NEN-7510:2024-2 controls aan een traceerbaar
...
\textbf{A.8.9} Configuratie\-beheer &
Gap gedocumenteerd in gap-analyse: \texttt{SettingsFormController\allowbreak
.searchProperties()} mist expliciete authenticatiecheck; niet empirisch
geverifieerd via pentest. Aanbeveling: alsnog testen of het endpoint in de
draaiende applicatie bereikbaar is zonder sessie. &
\texttt{docs/security/01-security-audit/01.md} \S3.5
(gap-analyse A.8.9); \texttt{SettingsFormController.java:50--63}. &
\amber{Gap (niet geverifieerd)} \\
\addlinespace

\textbf{A.8.29} Beveiligings\-tests &
Pentest uitgevoerd op OpenMRS 2.8.7 (Kali Linux, OWASP WSTG v4.2): 9
bevindingen (F-01 t/m F-09) gedocumenteerd en geprioriteerd; 1~kritisch (geen
rate limiting, authenticatiecredentials achterhaald in 6 pogingen), 4~hoog,
2~medium, 2~laag. Hervalidatie na mitigaties deels in uitvoering. &
Bijlage~\ref{app:pentest} (volledig pentestrapport); bevindingen F-01 t/m
F-09; bronbestand \texttt{05-penetration-tests/PentestReport-OpenMRS.md}. &
Geïmplementeerd \\
