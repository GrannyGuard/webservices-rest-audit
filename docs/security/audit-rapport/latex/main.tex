...
\vspace{1em}
De analyse leverde \textbf{tweeëntwintig gescoorde risico's} op uit vijf bronnen ---
SonarQube (SQ), CI/CD (CD), CodeQL (CQL), het threat model (AT) en de
penetratietest (PT). Twintig daarvan vallen in Midden of hoger; dertien zijn
\emph{onacceptabel} (zes maal \rood{Hoog} plus zeven Midden-risico's die
KJ1/KJ2/KJ3 met score $\geq 10$ raken). De volgende tabel toont de top van de
matrix (score $\geq 12$); de volledige tweeëntwintig staan in \texttt{01.md} \S4.2
en de pentestbevindingen in Bijlage~\ref{app:pentest}.
...
\addlinespace
\textbf{PT2} & Gebruikersenumeratie via API en lockout side-channel & \amber{12}$^\triangle$ & \amber{Midden} & KJ3 \\
\addlinespace
...
en \texttt{CD1} (supply chain, score~15). Eén risico scoort hoger dan de bow-tie-onderwerpen:
\texttt{PT1} (geen rate-limiting, score~20), empirisch bevestigd door de penetratietest
...
\textbf{Status}      & \rood{Open --- directe actie (Nu)} \\
\end{tabular}
...
\textbf{Gedeeltelijke mitigatie.} De fall-through na sessietime-out
(\texttt{return} ontbrak na \texttt{sendError(401)}, CWE-302) is hersteld
in commit \texttt{06.md} §3 --- 12 geautomatiseerde filtertests slagen.
De sessie-fixatie (CWE-384) staat nog open.

\textbf{Aanbeveling.} Sessie-ID regenereren na login (\texttt{session.invalidate()}
+ nieuwe sessie); op termijn declaratieve autorisatie (Spring Security)
ter vervanging van het best-effort filter.
De penetratietest heeft sessie-fixatie empirisch bevestigd: het
\texttt{JSESSIONID}-cookie was identiek vóór en ná authenticatie
(zie Bijlage~\ref{app:pentest}, F-03).

\subsection{B-003 --- Gebruikersenumeratie via API en lockout side-channel (PT2 / F-02)}
\label{sec:b003}
...
\textbf{Beschrijving.} Twee onafhankelijke methoden maken gebruikersenumeratie
mogelijk. \textbf{Methode A:} het endpoint
\texttt{GET /openmrs/ws/rest/v1/user} retourneert bij geauthenticeerde toegang een
volledige lijst van alle gebruikersaccounts in het systeem, inclusief
gebruikersnamen. \textbf{Methode B:} bestaande accounts worden geblokkeerd na
circa~7 mislukte inlogpogingen, terwijl niet-bestaande accounts nooit geblokkeerd
worden. Dit gedragsverschil --- lockout vs.\ geen lockout --- lekt welke
gebruikersnamen geldig zijn zonder ooit in te loggen.

\textbf{Bewijs.} Pentest F-02 (11--12 juni 2026); beide methoden bevestigd:
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize, breaklines=true, frame=single]
# Methode A — directe API-enumeratie (met auth)
GET /openmrs/ws/rest/v1/user  -> lijst alle accounts incl. "admin"

# Methode B — lockout side-channel (zonder auth)
doctor  -> geen lockout (gebruiker bestaat niet)
admin   -> lockout actief na 7 pogingen (gebruiker bestaat)
\end{lstlisting}

\textbf{Impact.} Een aanvaller verkrijgt een complete lijst van geldige
gebruikersnamen. Dit maakt gerichte brute-force aanvallen (B-001, F-01)
...
\textbf{Aanbeveling.} Uniform gedrag voor bestaande en niet-bestaande accounts
(identieke responstiming en foutmelding); vertraging toepassen bij herhaalde
mislukte pogingen ook voor niet-bestaande gebruikersnamen; de
\texttt{/user}-API beperken zodat een volledige gebruikerslijst niet opvraagbaar
is voor reguliere sessies.
...
\midrule
\rood{Nu} &
    Rate limiting: max.~10 pogingen/min per bron-IP, HTTP~429,
    log \texttt{ACCESS\_BLOCKED} &
    B-001 & A.8.5 \\
\rood{Nu} &
    Sessie-ID regenereren na login (\texttt{session.invalidate()} + nieuwe sessie) &
    B-002 & A.8.5 \\
\rood{Nu} &
    Uniform gedrag voor bestaande/niet-bestaande accounts (identieke timing +
    foutmelding); \texttt{/user}-API inperken voor niet-admin sessies &
    B-003 & A.8.5 \\
\rood{Nu} &
    Globale \texttt{@ExceptionHandler}: stack traces intern loggen, generiek
    bericht + referentie-ID naar client sturen &
    B-005 & A.8.15 \\
\rood{Nu} &
    Quick-win patches: \texttt{snakeyaml} $\to$ 2.0, \texttt{spring-web} $\to$ 5.3.34 &
    SBOM & A.8.8 \\
\amber{Deze sprint} &
    XSS-fix valideren via CodeQL-herscan; alert~\#1 op \texttt{state: fixed} &
    B-004 & A.8.28 \\
\amber{Deze sprint} &
    Non-breaking patches: \texttt{netty-handler}~4.1.119+, \texttt{c3p0}~0.10.x,
    \texttt{log4j-core} latest &
    SBOM & A.8.8 \\
\geel{Later} &
...
\textbf{PT2}  & Gebruikersenumeratie via API en lockout side-channel & 4 & 3 & \amber{12}$^\triangle$ & \amber{Midden}   & KJ3 \\
...
PT1 overschrijdt die drempel al op eigen score (20). Van de tweeëntwintig
risico's vallen er twintig in Midden of hoger; dertien zijn onacceptabel
(zes \rood{Hoog} plus zeven $^\triangle$-gemarkeerde Midden-risico's).\normalsize
...
gekoppeld aan SQ*/TM-*-risico's en B-001 t/m B-005. Hervalidatie deels in uitvoering. &
Bijlage~\ref{app:pentest}; \texttt{05-penetration-tests/PentestReport-OpenMRS.md} (F1--F9);
\texttt{zap-report.html}. &
\amber{Gedeeltelijk geverifieerd} \\
\addlinespace
...
\begin{itemize}
    ...
    \item Voor A.8.29: hervalidatie-pentest na mitigaties toevoegen als
          aanvullend bewijsartefact zodra de hertest is uitgevoerd.
\end{itemize}
...
F-01 & Geen rate limiting op authenticatie &
    CWE-307 & \rood{Kritiek} & A.8.5 & \rood{Open} \\
F-02 & Gebruikersenumeratie via API en lockout-gedrag &
    CWE-204 & \amber{Hoog} & A.8.5 & \rood{Open} \\
F-03 & Session fixation --- JSESSIONID niet geroteerd na login &
    CWE-384 & \amber{Hoog} & A.8.3 & \rood{Open} \\
F-04 & Stack trace disclosure in foutrespons &
    CWE-209 & \amber{Hoog} & A.8.15 & \rood{Open} \\
F-05 & Versie-informatie via \texttt{/systeminformation} &
    CWE-200 & \amber{Hoog} & A.8.15 & \rood{Open} \\
F-06 & Integer overflow in pagineringsparameter &
    CWE-190 & \geel{Middel} & A.8.28 & \rood{Open} \\
...
F-08 & Ontbrekende security headers &
    CWE-693 & Laag & A.8.23 & \rood{Open} \\
F-09 & Cookie zonder SameSite-attribuut &
    CWE-1275 & Laag & A.8.23 & \rood{Open} \\
...
Van de 9 bevindingen is F-07 (log injection) gemitigeerd via commit \texttt{831dafb}
(CodeQL 9 \textrightarrow{} 0 alerts, zie Bijlage~\ref{app:codeql} en
bevinding B-006 in Hoofdstuk~\ref{ch:bevindingen}). De overige 8 bevindingen zijn open. De meest urgent aan te
pakken zijn F-01 (kritiek, B-001), F-03 (sessie-fixatie, B-002) en F-04
(stack traces, B-005), conform de geprioriteerde aanbevelingen in
