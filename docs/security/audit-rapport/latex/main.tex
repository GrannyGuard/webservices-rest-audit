...
\textbf{OWASP}       & A09:2021 Security Logging and Monitoring Failures (CWE-209) \\
...
\begin{itemize}
    \item \textbf{Automatisch} --- de bots \texttt{github-advanced-security}
          ...
          analyseresultaat rechtstreeks als PR-commentaar. De verplichte status
          checks --- met de CodeQL-scan als mergeblokkerende gate --- en de
          review vormen de harde merge-voorwaarde; de SonarCloud-quality
          gate-uitslag wordt daarbij als afwegingsmoment meegewogen.
...
SAST-tool. Het draait op elke pull request en publiceert de \emph{quality
gate}-uitslag als check; het team behandelt een falende gate als expliciet te
documenteren afwegingsmoment (zie \texttt{04-code-review/04.md} \S1). De
mergeblokkerende, verplichte status check is op dit moment de CodeQL-scan; de
SonarCloud-gate is (nog) niet als required check ingesteld, omdat hij op
\texttt{main} nog faalt op de reliability-rating. Onderstaand dashboard toont de
uitgangsmeting bij aanvang van de mitigatiefase; de eindstand (hotspots naar 0,
Security Rating naar C) staat in de module-validatie van
\texttt{06-mitigatie-en-validatie/06.md}.

\begin{figure}[h]
    \centering
    \auditfig{../../04-code-review/Images/bijlage-sonarcloud-security.png}{0.95}
    \caption{SonarCloud-dashboard van de module bij de audit-uitgangsmeting: de
             quality gate staat op \emph{Failed} (3 condities), met \emph{Security
             Rating E} en twee security-issues (50\,\% Blocker, 50\,\% High). Dit
             beeld onderbouwt de SQ-reeks uit de risicomatrix
             (\S\ref{sec:risicomatrix}). Na de mitigaties is de stand verbeterd:
             Security Hotspots naar 0 (Security Review Rating A), Security Rating
             naar C en testdekking naar 66\,\%; de gate faalt op \texttt{main} nu
             nog op één conditie --- de reliability-rating (D) --- en niet langer
             op security (zie de module-validatie in \texttt{06.md}).}
...
gelogd. &
Commit \texttt{8521cdc}; 19 geautomatiseerde tests (12 + 7) in
