...
NEN-7510:2024-2 Deel 2, met als kern de zeven controls uit het Normenkader
(\S\ref{sec:normenkader}: A.8.3, A.8.5, A.8.8, A.8.9, A.8.15, A.8.24, A.8.25)
plus zeven aanvullende controls die tijdens de audit zelf zijn
geïmplementeerd en getest (Bijlage~\ref{app:traceability}, 14 controls
totaal, elk met een traceerbaar bewijsartefact). De audit combineert
gap-analyse, CIA/BIV- en risicoanalyse, STRIDE-threatmodelling, SAST
(CodeQL) en SCA (Dependabot/OSV-Scanner/Snyk) met een penetratietest
(F1--F9, Bijlage~\ref{app:traceability} A.8.29).

Van de vijf uitgewerkte bevindingen (B-001 t/m B-005, \S\ref{sec:bevindingen})
zijn er \textbf{twee volledig gemitigeerd en geverifieerd} (B-004 ---
reflected XSS; B-005 --- log injection) en is \textbf{één deels gemitigeerd}
(B-002: de CWE-302 fall-through in \texttt{AuthorizationFilter} is gefixt, de
onderliggende sessie-fixatie CWE-384 staat nog open). De twee resterende
bevindingen --- B-001 (ongeauthenticeerde uitlezing van alle
systeemconfiguratie, risicoscore 20, hoogste van de analyse) en B-003 (geen
brute-force-bescherming op authenticatie) --- zijn \textbf{nog open} en
vormen de hoogste prioriteit.

\textbf{Eindoordeel per control:} A.8.5 (Authenticatie) en A.8.9
(Configuratiebeheer) zijn \rood{onvoldoende}; A.8.3 (Toegangsbeveiliging) en
A.8.24 (Cryptografie) zijn \amber{gedeeltelijk}; A.8.15 (Logging) is
\amber{gedeeltelijk gemitigeerd} (auth-logging is op orde, resource-level
audit trail ontbreekt nog); A.8.8 (Kwetsbaarhedenbeheer) en A.8.25 (Veilige
ontwikkelcyclus) zijn \groen{geïmplementeerd}, evenals de zeven in Sprint 3
toegevoegde controls (least-privilege database, Host-validatie,
supply-chain-signing, secure coding en OTAP-scheiding).
...
\begin{tabularx}{\textwidth}{@{}l X l l@{}}
    ...
    \midrule
    B-001 & Iedereen kan --- zonder in te loggen --- alle systeeminstellingen
            opvragen, mogelijk inclusief opgeslagen wachtwoorden en de
            beveiligingsconfiguratie zelf. & \rood{20 --- Hoog} &
            \rood{Open --- prio 1} \\
    \addlinespace
    B-002 & Een aanvaller die een sessie-ID ``plant'' kan na een succesvolle
            login van een slachtoffer die sessie overnemen (sessie-fixatie). &
            \rood{15 --- Hoog} & \amber{Deels gemitigeerd} \\
    \addlinespace
    B-003 & Inlogpogingen worden niet beperkt: wachtwoorden raden
            (brute force/credential stuffing) is onbeperkt mogelijk. &
            \rood{Hoog} & \rood{Open --- prio 3} \\
    \addlinespace
    B-004 & Een gemanipuleerde link kon kwaadaardige code laten uitvoeren in de
            ...
            \groen{Gemitigeerd} \\
    \addlinespace
    B-005 & Logregels konden door een aanvaller worden vervalst
...
\begin{tabularx}{\textwidth}{@{}l X l@{}}
    ...
    \midrule
    \textbf{Nu} & B-001: \texttt{settings.form/search} achter de
                  \texttt{AuthorizationFilter}-keten plaatsen en een
                  privilege-check afdwingen (of verwijderen indien zonder
                  productiefunctie). & Direct \\
    \addlinespace
    \textbf{Nu} & B-003: rate-limiting/account-lockout invoeren op de
                  authenticatie-keten, met \texttt{ACCESS\_BLOCKED}-logging
                  bij overschrijding. & Direct \\
    \addlinespace
    \textbf{Deze sprint} & B-002: sessie-ID regenereren na login
                  (\texttt{session.invalidate()} + nieuwe sessie); daarna
                  pentest-hervalidatie van de B-002/B-004-keten. & 1 sprint \\
    \addlinespace
    \textbf{Deze sprint} & SBOM-quickwins doorvoeren: \texttt{snakeyaml}
                  convergeren naar 2.x, \texttt{spring-web} naar 5.3.34
                  (03.md \S7). & 1 sprint \\
    \addlinespace
    \textbf{Deze sprint} & CodeQL-herscan als bewijsartefact toevoegen voor
                  B-004 (alert \#1 $\to$ \texttt{fixed}), analoog aan B-005. & 1 sprint \\
    \addlinespace
    \textbf{Later} & A.8.24: TLS-afdwinging op deployment-niveau documenteren
                  en verifiëren (reverse proxy/servlet-container). & Roadmap \\
    \addlinespace
    \textbf{Later} & Declaratieve autorisatie (Spring Security) ter vervanging
                  van het best-effort \texttt{AuthorizationFilter}. & Roadmap \\
    \addlinespace
    \textbf{Later} & A.8.2/A.8.3: prod-runtime-switch naar het DML-only
                  database-account container-verifiëren. & Roadmap \\
    \addlinespace
    \textbf{Later} & Majeure SCA-patches (Spring~6, \texttt{jackson-databind}
                  2.x) gefaseerd inplannen (03.md \S6). & Roadmap \\
    \bottomrule
\end{tabularx}
}
...
\begin{itemize}
    ...
    \item \textbf{Fysieke beveiliging en organisatorische maatregelen} ---
          ...
          niet beoordeeld.
    \item \textbf{Penetratietests op productie} --- de penetratietest
          (ZAP-scan + handmatige tests, bevindingen F1--F9, zie
          Bijlage~\ref{app:traceability} A.8.29 en
          \texttt{05-penetration-tests/PentestReport-OpenMRS.md}) is
          uitsluitend uitgevoerd op de eigen development/test-fork; een
          geautoriseerde test tegen een productie(-achtige) omgeving viel
          buiten de scope van deze audit.
\end{itemize}
...
spreiding over de geauditeerde controls. Twee staan nog volledig open (B-001,
B-003), B-002 is deels gemitigeerd (de CWE-302 fall-through is gefixt, de
sessie-fixatie CWE-384 staat nog open), en B-004 en B-005 zijn gemitigeerd ---
B-005 toont een volledig afgesloten detect-\,$\to$\,mitigeer-\,$\to$\,verifieer-cyclus.
...
\subsection{B-004 --- Reflected XSS in het Swagger debug-endpoint (SQ8, F2): gemitigeerd}
\label{sec:b004}
...
\textbf{Validatie (voor/na).} Payload \texttt{?tag=<script>alert(1)</script>}
wordt nu gerenderd als platte tekst (\texttt{\&lt;script\&gt;...}) i.p.v.
uitgevoerd; \texttt{mvn clean install} groen, geen regressie. Zie
\texttt{06-mitigatie-en-validatie/06.md} \S2 voor het volledige voor/na-overzicht.

\textbf{Restpunt.} De CI-CodeQL-herscan die alert \#1 op \texttt{state: fixed}
zet moet nog als bewijsartefact worden toegevoegd (zelfde patroon als B-005).
...
aanvankelijk als \emph{``false positive''} gedismissed \emph{zonder} onderbouwing.
Omdat die classificatie afhing van de (in deze module afwezige) log-appender-config
en dus niet aantoonbaar was, zijn de user-controlled waarden (request-URI,
\texttt{attemptedUser}, exception-message) alsnog via
\texttt{RestUtil.sanitizeForLog()} gesanitized; de alerts sluiten op de
eerstvolgende CodeQL-herscan (bewijsartefact nog toe te voegen).
...
(\S\ref{sec:normenkader}) voldoet de module aan twee (A.8.8, A.8.25), is er
bij drie sprake van gedeeltelijke compliance (A.8.3, A.8.15, A.8.24) en
schiet de module bij twee tekort (A.8.5, A.8.9) --- zie
\S\ref{sec:overall-status} voor het overzicht en
Bijlage~\ref{app:traceability} voor de onderliggende bewijsvoering. De twee
tekortkomingen zijn direct gekoppeld aan de twee nog openstaande bevindingen
B-001 (risicoscore 20, hoogste van de analyse) en B-003. Daarnaast zijn
tijdens deze audit zeven \emph{aanvullende} controls geïmplementeerd en
getest (least-privilege database, Swagger/Host-validatie,
supply-chain-signing, secure coding, OTAP-scheiding en beveiligingstests) die
niet in het oorspronkelijke Normenkader stonden maar wel uit de
risicoanalyse volgden.
...
\textbf{Samenvattend:} de module is op dit moment \rood{niet volledig
compliant} met NEN-7510:2024-2, primair door B-001 (configuratiebeheer,
A.8.9, score 20) en de authenticatie-gaps (A.8.5: B-002 deels open, B-003
volledig open). De CRA-gerelateerde eisen aan supply-chain-transparantie
worden grotendeels ondersteund door de SBOM/SCA/signing-keten die tijdens
deze audit is opgezet en geverifieerd. Met de in
\S\ref{sec:roadmap} geprioriteerde roadmap is volledige compliance op de
geauditeerde controls binnen 1--2 sprints realistisch.

\section{Geprioriteerde Aanbevelingen}
\label{sec:aanbevelingen}

De aanbevelingen uit \S\ref{sec:roadmap} zijn geprioriteerd volgens de
volgende criteria, afgeleid uit de risicocriteria in \texttt{01.md}
\S2.3:

{\small
\begin{tabularx}{\textwidth}{@{}l X X@{}}
    \toprule
    \textbf{Prioriteit} & \textbf{Criterium} & \textbf{Actie} \\
    \midrule
    \textbf{Nu} & Kritieke of hoge bevinding nog volledig open
                  (risicoscore $\geq 15$): B-001 (20), B-003 (Hoog). &
                  Direct oplossen, vóór de volgende release; geen
                  productie-uitrol met deze bevindingen open. \\
    \addlinespace
    \textbf{Deze sprint} & Gemiddelde bevinding, of een hoge bevinding die al
                  deels gemitigeerd is: B-002 (CWE-302 gefixt, CWE-384 open),
                  SBOM-quickwins, CodeQL-herscan-artefact voor B-004. &
                  Inplannen in de lopende/volgende sprint; bij B-002 incl.\
                  pentest-hervalidatie van de B-002/B-004-keten. \\
    \addlinespace
    \textbf{Later} & Lage bevinding of structurele/architecturale verbetering
                  zonder direct exploiteerbaar risico: A.8.24
                  (TLS-afdwinging op deploymentniveau), declaratieve
                  autorisatie (Spring Security), prod-runtime-switch
                  container-verifiëren, majeure SCA-upgrades (Spring~6,
                  \texttt{jackson-databind}~2.x). &
                  Opnemen in de security-roadmap voor de volgende
                  sprints/kwartalen; geen blokkade voor de huidige release. \\
    \bottomrule
\end{tabularx}
}
...
alleen de top (score $\geq 12$) toonde: alle twintig gescoorde risico's
(\S\ref{app:matrix-full}) plus de twee bow-tie-analyses voor de hoogst scorende
...
\section*{E.1\quad Volledige risicomatrix (20 risico's)}
\label{app:matrix-full}

De risico's komen uit vier bronnen: \textbf{SQ} (SonarQube), \textbf{CD} (CI/CD),
\textbf{CQL} (CodeQL) en \textbf{AT} (threat model). Score $=$ Kans $\times$ Impact.
...
\textbf{SQ7}  & Ongeauth.\ disclosure global properties             & 4 & 5 & \rood{20}             & \rood{Hoog}        & KJ7, KJ4, KJ1--3 \\
...
SQ7 overschrijdt die drempel al op eigen score (20). Van de twintig risico's vallen
er achttien in Midden of hoger; elf zijn onacceptabel (zes \rood{Hoog} plus vijf
$^\triangle$-gemarkeerde Midden-risico's).\normalsize
...
SQ2 scoort nominaal hoger (16), maar is een code-kwaliteitsprobleem zonder concrete
aanvalsketen. \textbf{SQ1} is gekozen omdat het een exploiteerbare keten kent
(sessie-spoofing via \texttt{getRequestedSessionId()} in
\texttt{AuthorizationFilter.java}) én de meeste, hoogst geclassificeerde kroonjuwelen
raakt (KJ3, KJ1, KJ2). De preventieve en reactieve barrières corresponderen met
prioriteit 2, 3 en 6 van het geprioriteerd advies.
...
Deze bijlage koppelt \textbf{veertien} NEN-7510:2024-2 controls aan een
traceerbaar bewijsartefact (repository-instelling, CI-workflow, commit of
scan-output), conform de Sprint-4-vereiste van minimaal drie controls. De
eerste zes rijen dekken de controls uit het Normenkader
(\S\ref{sec:normenkader}) die als \emph{geïmplementeerd} zijn getraceerd;
\textbf{A.8.9} en \textbf{A.8.24} --- de resterende twee controls uit het
Normenkader --- zijn daar als \emph{gap} getraceerd (nog niet geïmplementeerd,
zie B-001 resp.\ de gap-analyse \S3.4 in \texttt{01.md}). De overige rijen
(A.8.29 en de Sprint-3-hardeningmaatregelen) zijn aanvullende controls die
buiten het oorspronkelijke Normenkader om zijn geïmplementeerd en getest. De
...
\textbf{A.8.9} Configuratiebeheer --- \emph{gap} &
\texttt{settings.form/search} retourneert alle global properties (incl.\
\texttt{REST\_ALLOWED\_IPS}) zonder authenticatie --- B-001/SQ7, risicoscore
20, hoogste van de analyse. Nog niet gemitigeerd. &
\texttt{01-security-audit/01.md} \S3.5 (gap-analyse); B-001
(\S\ref{sec:b001}); \texttt{SettingsFormController.java:50--63}. &
\rood{Open --- prio.\ 1} \\
\addlinespace
...
\textbf{A.8.29} Beveiligingstests &
Penetratietest op de development/test-fork: ZAP-scan + handmatige tests
tegen alle endpoint-categorieën; bevindingen F1--F9 gekoppeld aan de
SQ*/TM-*-risico's uit de threat-modelling en aan B-001 t/m B-005 hierboven. &
\texttt{05-penetration-tests/PentestReport-OpenMRS.md} (F1--F9);
\texttt{zap-report.html}; \texttt{endpoints.md}. &
...
\texttt{docker/db-init/10-runtime-least-privilege.sh},
\texttt{docker-compose.yml}, \texttt{docker/prod/docker-compose.yml};
...
\texttt{RestUtil.resolveSwaggerHost()}/\texttt{sanitizeScheme()}/
...
SHA-pinned (was 1/9). &
\texttt{.github/workflows/release-sign.yml}; SHA-pins in alle workflows;
...
scheme-sanitisatie (SQ9) en centrale CWE-117 log-injectie-neutralisatie
(\texttt{RestUtil.sanitizeForLog()}, CQL1). &
\texttt{RestUtil.java}; tests \texttt{SwaggerHardeningRestUtilTest} (8/8) +
\texttt{RestUtilTest}; CWE-117-fix commit \texttt{831dafb}. &
Geïmplementeerd en getest \\
\addlinespace
...
\texttt{docker/\{dev,test,prod\}/docker-compose.yml},
\texttt{docker/README.md}; \texttt{02.md} \S2. &
Geïmplementeerd \\
...
\begin{itemize}
    ...
    \item A.8.9 / A.8.24 (\emph{gap}-rijen) bijwerken naar
          ``Geïmplementeerd'' zodra B-001 (settings.form-autorisatie) resp.\
          de TLS-afdwinging zijn gerealiseerd.
    \item Voor A.8.20/A.8.26 (SQ9): de open SQ8-restpunt (CodeQL-herscan
          alert \#1 op \texttt{state: fixed}) als bewijsartefact toevoegen
          zodra de eerstvolgende CodeQL-run op \texttt{main} is afgerond.
